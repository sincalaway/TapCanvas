...
export const useRFStore = create<RFState>((set, get) => ({
  ...
  addNode: (type, label, extra) => set((s) => {
    ...
    const { label: extraLabel, autoLabel, ...restExtra } = rawExtra
  ...
  runSelected: async () => {
    ...
    if (kind === 'composeVideo' || kind === 'storyboard' || kind === 'video' || kind === 'tts' || kind === 'subtitleAlign' || kind === 'image' || kind === 'textToImage') {
