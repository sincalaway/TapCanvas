...
function getLogsDir() {
  const fromEnv = process.env.EVOLVER_LOGS_DIR;
  if (fromEnv) return resolveWithinWorkspace(fromEnv);
  return path.join(getWorkspaceRoot(), '.agents', 'logs');
}

function getMemoryDir() {
  const fromEnv = process.env.MEMORY_DIR || process.env.AGENTS_MEMORY_DIR;
  if (fromEnv) return resolveWithinWorkspace(fromEnv);
  return path.join(getWorkspaceRoot(), '.agents', 'memory');
}

function resolveWithinWorkspace(p) {
  const raw = String(p || '').trim();
  if (!raw) return getWorkspaceRoot();
  if (path.isAbsolute(raw)) return raw;
  return path.resolve(getWorkspaceRoot(), raw);
}
