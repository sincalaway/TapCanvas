...
`.trim();
  ...
  const isAgentsCliRuntime = !!(
    process.env.AGENTS_API_KEY ||
    process.env.AGENTS_PROFILE ||
    process.env.AGENTS_API_BASE_URL ||
    process.env.AGENTS_MEMORY_DIR ||
    process.env.AGENTS_REPO_PATH ||
    process.cwd().includes(`${path.sep}apps${path.sep}agents-cli`) ||
    __dirname.includes(`${path.sep}apps${path.sep}agents-cli${path.sep}`)
  );
  const reportDirective = isAgentsCliRuntime
    ? '6. Report: write a concise final plain-text summary (WHAT changed and WHY).'
    : '6. Report: Use `feishu-evolver-wrapper/report.js`. Describe WHAT/WHY.';
  const messagingConstraint = isAgentsCliRuntime
    ? '- Do not rely on external chat/report wrappers; report in plain text.'
    : '- No `exec` for messaging (use feishu-post/card).';
  const protectedSkillScopes = isAgentsCliRuntime
    ? '- NEVER modify `skills/evolver/` unless EVOLVE_ALLOW_SELF_MODIFY=true.'
  ...
  const protectedRoots = isAgentsCliRuntime
    ? '- NEVER delete root files: MEMORY.md, SOUL.md, IDENTITY.md, AGENTS.md, USER.md, HEARTBEAT.md, RECENT_EVENTS.md, TOOLS.md, agents.config.json, .env, package.json.'
  ...
  const protectedSkills = isAgentsCliRuntime
    ? '- NEVER delete/empty/overwrite: common, git-sync, evolver.'
