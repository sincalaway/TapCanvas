...
// Load environment variables from repo root and workspace root.
try {
...
const AGENT_SESSIONS_DIR =
  process.env.AGENT_SESSIONS_DIR ||
  process.env.AGENTS_SESSIONS_DIR ||
  path.join(MEMORY_DIR, 'sessions');
...
const WORKSPACE_ROOT = process.env.OPENCLAW_WORKSPACE || getWorkspaceRoot();
...
async function run() {
  ...
  const bridgeEnabled = (() => {
    const raw = String(process.env.EVOLVE_BRIDGE || '').trim().toLowerCase();
    if (raw === 'true' || raw === '1' || raw === 'yes') return true;
    if (raw === 'false' || raw === '0' || raw === 'no') return false;
    return !isAgentsCliRuntime;
  })();
  ...
  let reportingDirective = isAgentsCliRuntime
    ? `Report requirement:
  - In your final plain-text response (not a tool), include:
  ...
  if (bridgeEnabled) {
    ...
    const executorTask = [
      ...
      bridgeEnabled
        ...
        : 'Loop chaining: restart manually by running node skills/evolver/index.js run when needed.',
      'Example:',
      bridgeEnabled
    ...
    console.log('[BRIDGE DISABLED] Outputting protocol prompt for manual/agents-cli execution.');
    console.log(prompt);
