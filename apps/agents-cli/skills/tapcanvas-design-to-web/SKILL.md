...
description: 基于用户提供的设计稿图片还原真实网页。该 skill 可用于任意前端项目，但强依赖本地已启动且已授权的 TapCanvas vision 工作流；默认使用 `gemini-3.1-flash-image-preview` 做视觉取证，支持外部传入 vision prompt，且不可静默降级。
---
